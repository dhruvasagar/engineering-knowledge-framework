...
[x] **Solid Foundation** — The AI Engineering capability is fully built
    out across all nine document types, including AI-specific playbooks,
    templates and meta-AI workflows. Domain-specific AI workflows exist
    for Engineering, Architecture, Rails, Security, Quality and
    Accessibility. Remaining work focuses on agentic workflows,
    evaluation frameworks and deeper integration across all
    capabilities.
...
9. [x] Establish RFC and ADR processes for framework evolution (see [[file:./adr/][adr/]] directory and [[file:./templates/adr/README.org][ADR template]]).
